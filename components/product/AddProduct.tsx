...
export default function AddProduct({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (edit && product) {
      ...
      if (updatedProduct) router.push(`/product/view/${updatedProduct.id}`);

    } else {
      // else, create a new product
      const newProdut = await createProduct({ name, price, description, category });
      if (newProdut) router.push(`/search`);
    }
  };
    ...
    <div className="grid min-h-screen w-full max-w-4xl mx-auto px-4 md:px-6 py-20 md:py-24 gap-8">
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => router.push(`/product/${id}`)}>Cancel</Button>
